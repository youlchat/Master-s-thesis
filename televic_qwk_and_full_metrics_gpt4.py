...
# === Load Data ===
input_path = "/scratch/leuven/365/vsc36597/televic_data/final-essay/Summary_Cleaned_No_Zero_Scores.csv"
output_dir = "/scratch/leuven/365/vsc36597/televic_data/final-essay/result_images"

df = pd.read_csv(input_path)
df = df.dropna(subset=["Normalized_Teacher_Score", "Normalized_GPT_Score"])
df["Teacher"] = df["Normalized_Teacher_Score"]
df["GPT"] = df["Normalized_GPT_Score"]
df = df[(df["Teacher"] <= 1) & (df["GPT"] <= 1)]

def compute_metrics(sub_df):
    binary_t = (sub_df["Teacher"] >= 0.5).astype(int)
    binary_q = (sub_df["GPT"] >= 0.5).astype(int)
    try:
        pearson_corr, p_val = pearsonr(sub_df["Teacher"], sub_df["GPT"])
    except:
        pearson_corr, p_val = np.nan, np.nan
    try:
        auc = roc_auc_score(binary_t, sub_df["GPT"]) if len(np.unique(binary_t)) > 1 else np.nan
    except:
        auc = np.nan
    try:
    ...
    sub_df["Q_bin"] = (sub_df["GPT"] * 10).round().astype(int).clip(0, 10)
    try:
